...
class DicomClass:
    ...
    def DicomSelect(self,PathDicom):
        ...
        if (len(self.ser) > 0):
            ...
            for i in range(len(id)):
                temp = id[i]
                label[i] = ("%02d"%i) + " - (" + str(len(np.where(self.idx == i)[0])) + " Images)"+" Series " + str(temp)
        return label
    ...
    def resize(self):
        a=1.0
        z=np.zeros((np.round(self.dicomSizeMM[0]/a).astype(int),np.round(self.dicomSizeMM[1]/a).astype(int),self.dicomSizePixel[2]))
        zs=z.shape
        ...
        z = np.zeros((np.round(self.dicomSizeMM[0]/a).astype(int), np.round(self.dicomSizeMM[1]/a).astype(int), np.round(self.dicomSizeMM[2]/a).astype(int)))
        zs = z.shape
        for i in range(self.ct.shape[0]):
            tmp1=self.ct[i,:,:]
    ...
    def resize1(self):
        if self.scale[0]!=1.0:
            z=np.zeros((np.round(self.dicomSizePixel[0]*self.scale[0]).astype(int),self.dicomSizePixel[1],self.dicomSizePixel[2]))
            for i in range(self.ct.shape[2]):
                tmp1=self.ct[:,:,i]
                tmp2=scipy.misc.imresize(tmp1,(np.round(self.dicomSizePixel[0]*self.scale[0]).astype(int),self.dicomSizePixel[1]))
                z[:,:,i] = tmp2
            self.ct=z
        if self.scale[1]!=1.0:
            z=np.zeros((self.dicomSizePixel[0],np.round(self.dicomSizePixel[1]*self.scale[1]).astype(int),self.dicomSizePixel[2]))
            for i in range(self.ct.shape[2]):
                tmp1=self.ct[:,:,i]
                tmp2=scipy.misc.imresize(tmp1,(self.dicomSizePixel[0],np.round(self.dicomSizePixel[1]*self.scale[1]).astype(int)))
                z[:,:,i] = tmp2
            self.ct = z
        if self.scale[2]!=1.0:
            z=np.zeros((self.dicomSizePixel[0],self.dicomSizePixel[1],np.round(self.dicomSizePixel[2]*self.scale[2]).astype(int)))
            for i in range(self.ct.shape[0]):
                tmp1=self.ct[i,:,:]
                tmp2=scipy.misc.imresize(tmp1,(self.dicomSizePixel[1],np.round(self.dicomSizePixel[2]*self.scale[2]).astype(int)))
                z[i,:,:] = tmp2
            self.ct = z
