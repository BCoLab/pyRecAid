...
class MainForm(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.setZeroButton.move(233, 400)
        self.setZeroButton.clicked.connect(self.SetZero)
        self.setZeroButton.setEnabled(False)

        # button
        self.openLastReslice = QtWidgets.QPushButton("&Open Last Reslice", self)
        self.openLastReslice.move(67, 450)
        ...
        # region ReSlice
        # group box
        self.groupBox = QtWidgets.QGroupBox('&Reslice option', self)
        self.groupBox.move(67, 320)
        ...
        self.spinboxLabel1 = QtWidgets.QLabel("AP Angle:", self.groupBox)
        ...
        self.spinboxLabel2 = QtWidgets.QLabel("ML Angle:", self.groupBox)
        ...
        #button
        self.resliceButton= QtWidgets.QPushButton("&Reslice", self)
        self.resliceButton.move(67, 400)
        ...
        self.groupBoxch.move(67, 530)
        self.groupBoxch.resize(266, 90)
        ...
        #button
        self.addButton= QtWidgets.QPushButton("&Add Chamber", self)
        self.addButton.move(67, 650)
        self.addButton.clicked.connect(self.AddChamber)
        # self.addButton.setEnabled(False)

        self.clearChamber= QtWidgets.QPushButton("&Clear Chamber", self)
        self.clearChamber.move(233, 650)
        ...
        self.groupBoxDistance.move(1020, 480)
        ...
        self.groupBoxDistance1.move(1020, 690)
        ...
        self.labelImg00 = QtWidgets.QLabel("Transverse",self)
        self.labelImg00.move(652,20)
        ...
        resliceMenu = QtWidgets.QAction('Reslicing', self)
        resliceMenu.setStatusTip('reslicing')
        # resliceMenu.triggered.connect(self.ResliceMethod)
        resliceMenu.setEnabled(False)

        setZeroMenu = QtWidgets.QAction('Set Zero', self)
        setZeroMenu.setStatusTip('set zero')
        # setZeroMenu.triggered.connect(self.SetZero)
        setZeroMenu.setEnabled(False)

        addChamberMenu = QtWidgets.QAction('Add Chamber', self)
        addChamberMenu.setStatusTip('add chamber')
        # addChamberCenu.triggered.connect(self.AddChamber)
    ...
    def ResliceMethod(self):
        ...
        with self.WaitCursor():
            ...
            tempmap = scipy.ndimage.rotate(tempmap, a, axes=(1, 2), reshape=True, output=np.uint8, order=1, mode='constant', prefilter=True)
            tempmap = scipy.ndimage.rotate(tempmap, b, axes=(0, 2), reshape=True, output=np.uint8, order=1, mode='constant', prefilter=True)
            tempmap = scipy.ndimage.rotate(tempmap, c, axes=(0, 1), reshape=True, output=np.uint8, order=1, mode='constant', prefilter=True)

            newzero=(np.where(tempmap == np.max(tempmap))[0][0],np.where(tempmap == np.max(tempmap))[1][0],np.where(tempmap == np.max(tempmap))[2][0])
    ...
    def ShowDicom(self,ct,pos,frame):
        ...
        self.img2.setScaledContents(1)
        if frame==0:
            self.labelImg0.setText("(" + str(round((pos[0] - self.dicom.zeroPos[0])*self.dicom.scaleM[0],2)) + "mm) " + str(round(pos[0] * self.dicom.scaleM[0], 2)) + "mm /" + str(round(self.dicom.dicomSizeMM[0],2))+"mm.  "+"(" + str(pos[0] - self.dicom.zeroPos[0]) + ") " + str(pos[0]) + "/" + str(diSize[0]))
        if frame==1:
            self.labelImg1.setText("(" + str(round((pos[1] - self.dicom.zeroPos[1])*self.dicom.scaleM[1],2)) + "mm) " + str(round(pos[1] * self.dicom.scaleM[1], 2)) + "mm /" + str(round(self.dicom.dicomSizeMM[1],2))+"mm.  "+"(" + str(pos[1] - self.dicom.zeroPos[1]) + ") " + str(pos[1]) + "/" + str(diSize[1]))
        if frame == 2:
            self.labelImg2.setText("(" + str(round((pos[2] - self.dicom.zeroPos[2])*self.dicom.scaleM[2],2)) + "mm) " + str(round(pos[2] * self.dicom.scaleM[2], 2)) + "mm /" + str(round(self.dicom.dicomSizeMM[2],2))+"mm.  "+"(" + str(pos[2] - self.dicom.zeroPos[2]) + ") " + str(pos[2]) + "/" + str(diSize[2]))
    ...
    def DoubleClickEventImg0Event(self,event):
        if self.dicom is not None:
            if self.dicom.dicomData!=[]:
                ...
                a0=cv2.resize(a0, (round(self.dicom.scale[1] * self.dicom.dicomSizePixel[1]), round(self.dicom.scale[2] * self.dicom.dicomSizePixel[2])), interpolation = cv2.INTER_AREA)
    ...
    def DoubleClickEventImg1Event(self,event):
        if self.dicom is not None:
            if self.dicom.dicomData != []:
                ...
                a1=cv2.resize(a1, (round(self.dicom.scale[0] * self.dicom.dicomSizePixel[0]), round(self.dicom.scale[2] * self.dicom.dicomSizePixel[2])), interpolation = cv2.INTER_AREA)
    ...
    def DoubleClickEventImg2Event(self,event):
        if self.dicom is not None:
            if self.dicom.dicomData != []:
                ...
                a2=cv2.resize(a2, (round(self.dicom.scale[0] * self.dicom.dicomSizePixel[0]), round(self.dicom.scale[1] * self.dicom.dicomSizePixel[1])), interpolation = cv2.INTER_AREA)
    ...
    def SendClickedEvent(self):
        with self.WaitCursor():
            ...
            (a,b,c)=self.dicom.reslice
            # newpos= self.ComputeNewPos(np.shape(self.dicom.dicomData)[0], np.shape(self.dicom.dicomData)[1],
            #               np.shape(self.dicom.dicomData)[2], a,b,c,self.dicom.markerPos[0],self.dicom.markerPos[1],self.dicom.markerPos[2])

            tempmap = scipy.ndimage.rotate(tempmap, a, axes=(1, 2), reshape=True, output=np.uint8, order=5, mode='constant', prefilter=True)
            tempmap = scipy.ndimage.rotate(tempmap, b, axes=(0, 2), reshape=True, output=np.uint8, order=5, mode='constant', prefilter=True)
            tempmap = scipy.ndimage.rotate(tempmap, c, axes=(0, 1), reshape=True, output=np.uint8, order=5, mode='constant', prefilter=True)
            newpos=(np.where(tempmap == np.max(tempmap))[0][0],np.where(tempmap == np.max(tempmap))[1][0],np.where(tempmap == np.max(tempmap))[2][0])
    ...
    def AddChamber(self):
        try:
            ...
            chamberPixelSize = [self.chamber.xSize / self.dicom.scaleM[0], self.chamber.ySize / self.dicom.scaleM[1], self.chamber.zSize / self.dicom.scaleM[2]]
            ...
            # self.dicom.dicomData[chPos[0]:chPos[0] + chPixelSize[0], chPos[1]:chPos[1] + chPixelSize[1], chPos[2]:chPos[2] + chPixelSize[2]] = np.maximum(ch,self.dicom.dicomData[chPos[0]:chPos[0] + chPixelSize[0], chPos[1]:chPos[1] + chPixelSize[1], chPos[2]:chPos[2] + chPixelSize[2]])
            self.dicom.dicomData[chPos[0]- chPixelSize[0]:chPos[0] , chPos[1]- chPixelSize[1]:chPos[1] , chPos[2]- chPixelSize[2]:chPos[2] ] = \
                np.maximum(ch,self.dicom.dicomData[chPos[0]- chPixelSize[0]:chPos[0] , chPos[1]- chPixelSize[1]:chPos[1] , chPos[2]- chPixelSize[2]:chPos[2] ])
        except:
            msg = QtWidgets.QMessageBox()
            msg.setIcon(QtWidgets.QMessageBox.Information)
            msg.setWindowTitle("Error")
    ...
    def BeginDistanceButton1(self):
        ...
        self.beginDistanceLabel1.setText("Begin Distance:   "+str(self.beginDistance1[0])+" , "+str(self.beginDistance1[1])+" , "+str(self.beginDistance1[2]))
    ...
    def BeginDistanceButton(self):
        ...
        x=math.sqrt(math.pow(self.endDistance[0]-self.beginDistance[0],2)+math.pow(self.endDistance[1]-self.beginDistance[1],2)+math.pow(self.endDistance[2]-self.beginDistance[2],2))

        tmp=np.round(np.multiply(self.dicom.pos,self.dicom.scaleM),2)
        self.beginDistanceLabel.setText("Begin Distance:   "+str(tmp[0])+" , "+str(tmp[1])+" , "+str(tmp[2]))

        angle0 =math.atan2(self.endDistance[0] - self.beginDistance[0], self.endDistance[1] - self.beginDistance[1]) * 180 / math.pi
        angle1 =math.atan2(self.endDistance[0] - self.beginDistance[0], self.endDistance[2] - self.beginDistance[2]) * 180 / math.pi
